...
def test_datacite_preserves_doi_metadata_and_bounds_the_query(monkeypatch):
    ...
    assert params["query"] == (
        "titles.title:(agent benchmark) AND "
...
@pytest.mark.parametrize(
    ("registered", "updated"),
    [
        ("2026-07-29T09:00:00.000Z", "2026-07-29T09:00:05.000Z"),
        ("2026-07-27T09:00:00.000Z", "2026-07-29T09:00:00.000Z"),
    ],
    ids=["registered_ahead_of_the_run", "edited_ahead_of_the_run"],
)
def test_datacite_rejects_future_dated_records_and_counts_them(monkeypatch, registered, updated):
    # Either timestamp past the collection instant means an upstream clock the
    # record cannot be dated against, so an in-window `registered` does not
    # rescue a row whose `updated` is ahead of the run. Counting the rejection
    # matters as much as dropping it: run_pipeline adds `_future_rejections`
    # back into the fetched total, so a silent drop makes the day's funnel
    # stop adding up.
    monkeypatch.setattr(
        "benchmark_radar.sources.get_json",
        lambda url, **kwargs: _datacite_payload(registered=registered, updated=updated),
    )
...
def test_datacite_search_phrase_cannot_restructure_the_query(monkeypatch):
    ...
    assert calls[0]["query"] == (
        "titles.title:(Benchmark\\: agents \\(2026\\)) AND "
        "registered:[2026-07-26T12:00:00Z TO 2026-07-28T00:05:00Z]"
    )
...
def test_openaire_preserves_upstream_metadata_and_bounds_the_query(monkeypatch):
    ...
    # Crossref, DataCite or Zenodo merges with it instead of being published
    # a second time.
    assert item.url == "https://doi.org/10.5281/zenodo.99001"
...
def test_openaire_reduces_a_deposited_doi_to_its_bare_name(monkeypatch, deposited):
    ...
    # which resolves nowhere and shares no identity with the Crossref, DataCite
    # or Zenodo copy of the same artifact, so the record would be counted twice.
    monkeypatch.setattr(
        "benchmark_radar.sources.get_json",
